defmodule LiveFlightWeb.PostLive.Show do
  use LiveFlightWeb, :live_view

  alias LiveFlight.Blog
  ...
  @impl true
  def handle_info(
        {:updated, %LiveFlight.Blog.Post{id: id} = post},
  ...
  def handle_info(
        {:deleted, %LiveFlight.Blog.Post{id: id}},
        %{assigns: %{post: %{id: id}}} = socket
      ) do
    {:noreply,
     socket
  ...
  def handle_info({type, %LiveFlight.Blog.Post{}}, socket)
